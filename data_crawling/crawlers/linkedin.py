...
class LinkedInCrawler(BaseCrawler):
    ...
    def extract(self, link: str, **kwargs) -> None:
        ...
        self.model.bulk_insert(
            [
                PostDocument(
                    platform="linkedin", content=posts, author_id=kwargs.get("user", "")
                )
                for post in posts
            ]
        )
    ...
    def login(self) -> None:
        ...
        if settings.LINKEDIN_USERNAME is not None:
            self.driver.find_element(By.ID, "username").send_keys(
                settings.LINKEDIN_USERNAME
            )
        else:
            raise ValueError("LINKEDIN_USERNAME is None")

        if settings.LINKEDIN_PASSWORD is not None:
            self.driver.find_element(By.ID, "password").send_keys(
                settings.LINKEDIN_PASSWORD
            )
        else:
            raise ValueError("LINKEDIN_PASSWORD is None")
